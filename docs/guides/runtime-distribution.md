...
启动器会在配置路径相对于当前工作目录不存在时，根据可执行文件所在的 `bin/` 目录定位安装根目录，
再从安装根目录解析配置和模块路径。因此既可以进入解压后的顶层目录启动，也可以从任意工作目录启动。
...
验证该启动方式，也会从解压目录之外验证启动器定位安装根目录的行为。
...
可以。发行包中的启动器会在调用方工作目录找不到配置文件时定位安装根目录，并从该目录启动
Skynet。若调用方工作目录中已经存在同名配置文件，则保持 Skynet 原有行为，优先使用该文件。
