...
- 地址文本保持 Skynet Lua API 可解析格式，平台错误文本允许措辞不同，但必须非空。
...
- 定义私有 socket 状态、命令、事件和缓冲区所有权结构；对外仍只暴露整数 ID。
- ID 使用 16 位槽位和 15 位非零 generation，保证公开 `int` 始终为正，并为迟到回调校验保留完整代际。
- 增加 ID 编解码、generation 回绕、同槽不同代际和状态分类单元测试。
- 实现 mutex 保护的 MPSC FIFO、`uv_async_t` 合并唤醒、阻塞 poll、EXIT 事件和可重复 release；停止入队与 EXIT 在同一临界区完成。
...
- 实现单优先级异步 send；自有缓冲区转移给 write 请求，借用缓冲区在进入命令队列前复制。
- 覆盖连续写、多 worker 并发写、借用缓冲区修改隔离，以及写入过程中关闭时只释放一次。
