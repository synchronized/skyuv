...
| `client` | clientsocket、crypt、sha1 | 已构建、可加载 | Windows 保留原生 `SOCKET` 宽度；stdin 延迟启动并报告 EOF/错误/溢出 | 三平台使用同一驱动验证模块加载、connect/send/recv/shutdown/close、对端关闭和拒绝连接；Linux 另与上游对照；Windows 管道输入已验证 |
