...
|---|---|---|---|---|
| `skynet` | 核心、序列化、socket、mongo、netpack、memory、multicast、cluster、crypt、sharedata、stm、debugchannel、datasheet、sharetable | 已纳入核心、序列化、socket、crypt、stm、sharetable 和 sharedata | 多个源文件依赖 Skynet 内部符号；`lua-socket` 已完成 Winsock/VLA 适配；crypt 的 POSIX 随机函数已由兼容层提供；共享数据模块依赖内置 Lua 扩展和 skyuv 原子层 | crypt、stm 和 sharetable 已完成功能测试；sharedata 已验证只读代理、深拷贝、更新传播、旧代理失效及删除；其余模块分批验证 |
