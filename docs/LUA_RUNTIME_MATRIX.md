...
|---|---|---|---|---|
| `skynet` | 核心、序列化、socket、mongo、netpack、memory、multicast、cluster、crypt、sharedata、stm、debugchannel、datasheet、sharetable | 已纳入核心、序列化、socket、crypt、stm、sharetable、sharedata、datasheet、netpack 和 memory | 多个源文件依赖 Skynet 内部符号；`lua-socket` 已完成 Winsock/VLA 适配；crypt 的 POSIX 随机函数已由兼容层提供；便携 memory 后端不伪造 jemalloc 服务级统计 | memory API 已完整加载并验证，当前基础查询返回稳定零值、服务表为空、profiling 关闭；其余模块分批验证 |
